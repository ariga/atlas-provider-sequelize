...
const loadSQL = async (relativePath: string, driver: string): Promise<string> => {
    ...
    for (const model of models) {
        ...
        // @ts-ignore
        sql += sequelize.getQueryInterface().queryGenerator.createTableQuery(def.tableName, attr, Object.assign({}, def.options)) + "\n";
    }
    return sql
}
...
usage("npx @ariga/ts-atlas-provider-sequelize load --path ./models --dialect mysql")
    .alias("h", "help");
y.command(
        "load",
